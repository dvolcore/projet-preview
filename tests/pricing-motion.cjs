...
(async()=>{
 ...
 try{
  for(const width of [320,390,768,1440]){
   ...
   await page.emulateMedia({reducedMotion:'reduce'});await toggle.waitFor({state:'hidden'});
   ...
   await page.emulateMedia({reducedMotion:'no-preference'});await toggle.waitFor({state:'visible'});
